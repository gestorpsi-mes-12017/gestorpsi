...
class Place(models.Model):
    ...
    # Defines how many hours the room will be open
    # return array = [hour_start], [min_start], [hours_works]
    def hours_work(self):
        room_hours = [None]*3

        # hour
        room_hours[0] = (int(self.hour_start.split(',')[0]))
        # min
        room_hours[1] = (int(self.hour_start.split(',')[1]))

        available_hours = (float(self.availabe_schedules()))

        total_hours = (float(self.hour_end.split(',')[0]) -
        float(self.hour_start.split(',')[0])) + available_hours

        room_hours[2] = total_hours
        return room_hours

    def availabe_schedules(self):
        # how many avaiable schedules are availabe in the agenda
        available_hours = 0

        # Does exist half schedule?
        if int(self.hour_start.split(',')[1]) > int(self.hour_end.split(',')[1]):
            available_hours = float(-.5)
...
class RoomType(models.Model):
    """
    This class contains information on room types,
    thus instances of this class can be used to
...
class Room(models.Model):
    """
    This class represents a room, it also holds information on
    the furniture that belongs to the underlying room and its dimension.
    @version: 1.0
    """
    id = UuidField(primary_key=True)
