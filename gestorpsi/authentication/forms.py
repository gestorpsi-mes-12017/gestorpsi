...
class AuthenticationRegistrationForm(RegistrationRegistrationForm):
    ...
    username = forms.CharField(label=_('Username'), help_text=_('Choice one unique identifier'), widget=forms.TextInput(attrs={'required': 'true'}))
...
class RegistrationForm(AuthenticationRegistrationForm):
    ...
    plan = forms.ModelChoiceField(label=_('Access Plan'), help_text=_('Choice one access plan'), queryset=Plan.objects.filter(active=True), widget=forms.Select(attrs={'style':'width:265px;', 'required': 'true'}))
    ...
    state = forms.ModelChoiceField(label=_('State'), help_text=_('Enter your state here'), queryset=State.objects.all(), widget=forms.Select(attrs={'style':'width:265px;', 'class':'city_search', 'required': 'true'}))
    city = forms.ModelChoiceField(label=_('City'), help_text=_('Enter your city here'), queryset=City.objects.all(), widget=forms.Select(attrs={'style':'width:265px;', 'required': 'true'}))
